# C:\laragon\www\dapodik3\main_scraper.pyw - VERSI LENGKAP YANG DIPERBAIKI
import requests
...
# Konfigurasi yang ditingkatkan untuk ketahanan maksimal
JEDA_SAAT_GAGAL = 30  # Ditingkatkan menjadi 30 detik
JEDA_ANTAR_REQUEST = 5  # Jeda antara request untuk menghindari rate limiting
MAX_RETRIES = 100  # Ditingkatkan drastis untuk retry tanpa henti
REQUEST_TIMEOUT = 120  # Timeout ditingkatkan
...
def create_session_with_retry():
    """Membuat session requests dengan retry strategy yang sangat agresif"""
    session = requests.Session()
    
    # Retry strategy yang sangat agresif
    retry_strategy = Retry(
        total=10,
        backoff_factor=2,
        status_forcelist=[429, 500, 502, 503, 504, 408, 413],
    ...
    # Headers yang lebih realistis dengan rotasi
    user_agents = [
...
def get_urls_from_database(selected_ids=None, sekolah_scrape_id=None):
    """Mengambil URL dari database berdasarkan ID yang dipilih atau sekolah_scrape_id"""
    try:
        conn = get_db_connection()
        if not conn:
            return []
        ...
        if sekolah_scrape_id:
            # Mode langsung dengan sekolah_scrape_id
            query = "SELECT id, url, description FROM scraping_urls WHERE status = 'active' AND sekolah_scrape_id = %s"
            cursor.execute(query, (sekolah_scrape_id,))
        elif selected_ids:
            # Mode dari log dengan selected_ids
            ...
            # Fallback - ambil semua URL aktif (limited untuk safety)
            query = "SELECT id, url, description FROM scraping_urls WHERE status = 'active' LIMIT 50"
...
def create_simple_log_for_direct_mode(sekolah_scrape_id, user_id):
    """Buat log sederhana untuk mode langsung dengan sekolah_scrape_id - DIPERBAIKI"""
    try:
        conn = get_db_connection()
        if not conn:
            return None
        ...
        batch_name = f'Direct Mode - Sekolah ID {sekolah_scrape_id}'
        # Perbaikan: Hanya insert ke kolom yang diperlukan, tanpa id karena auto_increment
        cursor.execute("""
            INSERT INTO scraping_logs (batch_name, total_urls, url_ids, user_id, status, started_at) 
            VALUES (%s, 1, %s, %s, 'running', NOW())
        """, (batch_name, json.dumps([sekolah_scrape_id]), user_id))
        ...
        print(f"Error membuat log untuk mode langsung: {e}")
        traceback.print_exc()
        return None
...
def extract_sekolah_kita_data(html_content):
    """Extract data dari halaman Sekolah Kita dengan penanganan error yang lebih baik"""
...
def unduh_dan_scrape_sekolah(url_data, batch_dir, log_id, lock, counters):
    """Fungsi untuk mengunduh dan scraping data sekolah - VERSI SUPER KUAT"""
    global should_stop
    ...
    while not should_stop and retry_count < max_retries and not success:
        # Cek status log secara berkala
        if retry_count % 5 == 0 and log_id:  # Cek setiap 5 percobaan
        ...
        try:
            # Tampilkan progress
            print(f"[{thread_name}] Percobaan ke-{retry_count + 1}/{max_retries} untuk {url_sekolah}")
            
            # Request halaman dengan timeout dan retry
            halaman_response = session.get(url_sekolah, timeout=REQUEST_TIMEOUT)
            
            # Cek status code
            if halaman_response.status_code == 404:
                print(f"[{thread_name}] WARNING: URL tidak ditemukan (404), tetap akan mencoba lagi...")
                time.sleep(JEDA_SAAT_GAGAL * 2)  # Tunggu lebih lama untuk 404
                retry_count += 1
                continue
            ...
            if len(halaman_response.text) < 1000:
                print(f"[{thread_name}] WARNING: Halaman terlalu pendek, mungkin error, mencoba lagi...")
                retry_count += 1
                time.sleep(JEDA_SAAT_GAGAL)
                continue
            ...
            if not link_element:
                print(f"[{thread_name}] WARNING: Link unduh tidak ditemukan, mencoba lagi...")
                retry_count += 1
                time.sleep(JEDA_SAAT_GAGAL)
                continue
            ...
            if not nama_sekolah_element:
                print(f"[{thread_name}] WARNING: Nama sekolah tidak ditemukan, mencoba lagi...")
                retry_count += 1
                time.sleep(JEDA_SAAT_GAGAL)
                continue
        ...
        if retry_count < max_retries and not should_stop and not success:
            # Jeda progresif yang semakin lama
            base_wait = JEDA_SAAT_GAGAL
            if retry_count > 10:
                base_wait = JEDA_SAAT_GAGAL * 2
            if retry_count > 30:
                base_wait = JEDA_SAAT_GAGAL * 3
                
            wait_time = base_wait + random.uniform(0, 10)  # Random delay untuk menghindari pattern
            ...
            for i in range(int(wait_time * 10)):
                if should_stop or (log_id and check_log_status(log_id) == 'cancelled'):
                    should_stop = True
                    return
                time.sleep(0.1)
        elif not success:
...
# ================================================================
# SECTION 3: FUNGSI KONVERSI - DIPERBAIKI
# ================================================================

def convert_excel_to_json(downloaded_files):
    """Konversi file Excel ke JSON dengan penanganan error openpyxl - DIPERBAIKI"""
    converted_files = []
    ...
    try:
        ...
        print("  [INFO] Library openpyxl tersedia")
    except ImportError:
        print("  [WARNING] Library 'openpyxl' tidak ditemukan")
    
    try:
        import xlrd
        xlrd_available = True
        print("  [INFO] Library xlrd tersedia")
    except ImportError:
        print("  [WARNING] Library 'xlrd' tidak ditemukan")
    
    if not openpyxl_available and not xlrd_available:
        print("  [ERROR] Tidak ada library Excel yang tersedia!")
        print("  [SOLUSI] Silakan install salah satu:")
    ...
    for url_id, nama_sekolah, excel_path, info_json_path in downloaded_files:
        ...
        try:
            ...
            # Tentukan engine berdasarkan ekstensi file dan ketersediaan library
            file_ext = os.path.splitext(excel_path)[1].lower()
            
            if file_ext == '.xlsx':
                if openpyxl_available:
                    print(f"  [INFO] Menggunakan openpyxl untuk file {file_ext}")
                    dict_of_sheets = pd.read_excel(excel_path, sheet_name=None, header=None, engine='openpyxl')
                else:
                    print(f"  [ERROR] File {file_ext} memerlukan library openpyxl")
                    print(f"  [SOLUSI] Silakan install: pip install openpyxl")
                    continue
            elif file_ext == '.xls':
                if xlrd_available:
                    print(f"  [INFO] Menggunakan xlrd untuk file {file_ext}")
                    dict_of_sheets = pd.read_excel(excel_path, sheet_name=None, header=None, engine='xlrd')
                elif openpyxl_available:
                    print(f"  [INFO] Menggunakan openpyxl untuk file {file_ext}")
                    dict_of_sheets = pd.read_excel(excel_path, sheet_name=None, header=None, engine='openpyxl')
                else:
                    print(f"  [ERROR] File {file_ext} memerlukan library xlrd atau openpyxl")
                    print(f"  [SOLUSI] Silakan install: pip install xlrd")
                    continue
            else:
...
def main():
    """Fungsi utama scraper - VERSI SUPER KUAT"""
    global log_id
    
    # Parser yang diperbaiki
    parser = argparse.ArgumentParser(description="Scraper data sekolah Dapodik.")
    
    # Tambahkan kedua opsi tanpa mutually_exclusive_group
    parser.add_argument('--log_id', type=int, help='ID log proses dari database (mode normal)')
    parser.add_argument('--sekolah_scrape_id', type=int, help='ID sekolah scrape untuk mode langsung')
    parser.add_argument('--user_id', type=int, required=True, help='User ID performing the scraping')
    
    args = parser.parse_args()
    
    # Validasi manual: salah satu harus ada
    if not args.log_id and not args.sekolah_scrape_id:
        parser.error("Salah satu dari --log_id atau --sekolah_scrape_id harus diberikan")
    
    if args.log_id and args.sekolah_scrape_id:
        parser.error("Hanya boleh menggunakan salah satu: --log_id atau --sekolah_scrape_id")
    
    try:
        # Tentukan mode operasi
        if args.log_id:
            # Mode normal dengan log_id
            log_id = args.log_id
            print(f"Proses dimulai untuk log_id: {log_id}")
            selected_url_ids = get_task_details_from_log(log_id)
            url_list = get_urls_from_database(selected_ids=selected_url_ids)
        else:
            # Mode langsung dengan sekolah_scrape_id
            sekolah_scrape_id = args.sekolah_scrape_id
            print(f"Proses dimulai untuk sekolah_scrape_id: {sekolah_scrape_id}")
            log_id = create_simple_log_for_direct_mode(sekolah_scrape_id, args.user_id)
            url_list = get_urls_from_database(sekolah_scrape_id=sekolah_scrape_id)
        
        if not url_list:
            raise Exception("Tidak ada URL yang ditemukan untuk diproses.")

        # Setup folder dan update PID
        ...
        print(f"MAX RETRIES: {MAX_RETRIES} kali")
        print(f"JEDA SAAT GAGAL: {JEDA_SAAT_GAGAL} detik")
