...
def run_process(script_name, url_induk_id=None, kecamatan_id=None, max_retries=300):
    ...
    try:
        ...
        if kecamatan_id:
            cmd.extend(['--kecamatan_id', str(kecamatan_id)])
        if max_retries:
...
def main():
    ...
    parser.add_argument('--kecamatan_id', type=int, help='Kecamatan ID to process')
    ...
    logger.info(f"Kecamatan ID: {args.kecamatan_id}")
    ...
    for script, description in processes:
        ...
        if run_process(script, args.url_induk_id, args.kecamatan_id, args.max_retries):
            success_count += 1
