...
try {
    ...
    $query = "
        SELECT 
            ...
            COALESCE(sl.status, 'idle') AS user_status
        FROM 
            scraping_urls su
        LEFT JOIN sekolah_scrape ss ON su.sekolah_scrape_id = ss.id
        LEFT JOIN kecamatan_scrape ks ON ss.kecamatan_scrape_id = ks.id
        LEFT JOIN scraping_logs sl 
               ON su.id = sl.scraping_url_id 
