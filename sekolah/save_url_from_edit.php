...
try {
    ...
    if ($existing_url_id > 0) {
        ...
        // Check if URL already exists for other records of this user
        $check_query = mysqli_query($connection, 
            "SELECT id FROM scraping_urls 
             WHERE url = '$escaped_url' AND id != $existing_url_id AND user_id = $user_id");
        
        if (mysqli_num_rows($check_query) > 0) {
            throw new Exception('URL ini sudah digunakan untuk sekolah lain');
        }
        
        // Update the existing record for this user
        $update_query = "UPDATE scraping_urls 
                        SET url = '$escaped_url', 
                            description = '$escaped_description',
                            updated_at = NOW()
                        WHERE id = $existing_url_id AND user_id = $user_id";
        ...
        $check_query = mysqli_query($connection, 
            "SELECT id FROM scraping_urls WHERE url = '$escaped_url' AND user_id = $user_id");
        
        if (mysqli_num_rows($check_query) > 0) {
            throw new Exception('URL ini sudah ada di database');
        }
        
        // Insert new URL for this user
        $insert_query = "INSERT INTO scraping_urls (user_id, url, description, status) 
                        VALUES ($user_id, '$escaped_url', '$escaped_description', 'active')";
