<?php
// FILE: sekolah/check_scraping_progress.php
...
try {
    ...
    if ($log_id == 0) {
        $stmt = $connection->prepare("SELECT * FROM scraping_logs WHERE status = 'running' AND user_id = ? ORDER BY started_at DESC LIMIT 1");
        $stmt->bind_param("i", $user_id);
    } else {
        $stmt = $connection->prepare("SELECT * FROM scraping_logs WHERE id = ? AND user_id = ?");
