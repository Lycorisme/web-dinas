...
try {
    ...
    foreach ($urls as $index => $url_data) {
        ...
        $check_query = "SELECT id FROM scraping_urls WHERE url = ? AND user_id = ?";
        $check_stmt = mysqli_prepare($connection, $check_query);
        mysqli_stmt_bind_param($check_stmt, 'si', $url, $user_id);
        ...
        $insert_query = "INSERT INTO scraping_urls (user_id, url, description, status, created_at, updated_at) VALUES (?, ?, ?, 'active', NOW(), NOW())";
        $insert_stmt = mysqli_prepare($connection, $insert_query);
        mysqli_stmt_bind_param($insert_stmt, 'iss', $user_id, $url, $description);
