...
<a href="javascript:void(0);" class="btn btn-danger" 
   ...
   onclick="confirmDelete('<?= $data['id'] ?>', '<?= htmlspecialchars($data['nama_sekolah']) ?>', 'pelengkap')">
 ...
 $(document).ready(function() {
    ...
    $('#table-1').DataTable({
        ...
        columnDefs: [
            { 
                orderable: false, 
                targets: [0, -1] // Kolom no dan aksi tidak bisa di-sort
            },
            {
                className: "text-center",
                targets: [0, -1] // Kolom no dan aksi di tengah
            }
        ],
