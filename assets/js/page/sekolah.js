...
function startScraping(mode, skipConfirmation = false) {
  let selectedUrlIds = [];
  if (mode === 'selected') {
  ...
  if (!skipConfirmation) {
    let title, message;
    if (mode === 'selected') {
      title = 'Konfirmasi Update Terpilih';
      message = `Anda akan mengupdate data dari ${selectedUrlIds.length} URL terpilih. Lanjutkan?`;
    } else {
      title = 'Konfirmasi Update Semua';
      message = 'Yakin ingin memulai update untuk SEMUA URL aktif?';
    }
...
function stopScraping(skipConfirmation = false) {
        ...
        if (data.success) {
          ...
          if (['completed', 'failed', 'cancelled'].includes(data.status)) {
            ...
            if (data.status === 'completed') {
              ...
              setTimeout(() => {
                // Gunakan SweetAlert2 untuk konfirmasi reload
                Swal.fire({
                  title: 'Proses Selesai',
                  text: 'Muat ulang halaman untuk melihat data terbaru?',
                  icon: 'question',
                  showCancelButton: true,
                  confirmButtonColor: '#3085d6',
                  cancelButtonColor: '#d33',
