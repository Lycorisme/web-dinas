...
// Garis bawah setelah header
 $sheet->getStyle('A5:L5')->getBorders()->getBottom()->setBorderStyle(Border::BORDER_THICK);

// Judul laporan
 $sheet->setCellValue('A7', 'Rekapitulasi Data PTK (Pendidik dan Tenaga Kependidikan)');
...
if (empty($data_ptk)) {
    ...
    $sheet->setCellValue('A11', 'Tidak ada data PTK yang sesuai dengan filter yang dipilih');
    ...
    $sheet->setCellValue('H11', 'Total Guru');
    $sheet->setCellValue('I11', 'Total Tendik');
    $sheet->setCellValue('J11', 'Total PTK');
    $sheet->setCellValue('K11', 'PTK Laki-laki');
    $sheet->setCellValue('L11', 'PTK Perempuan');

    // Style header tabel
    $sheet->getStyle('A11:L11')->getFont()->setBold(true);
    $sheet->getStyle('A11:L11')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
        ->getStartColor()->setARGB('FFE0E0E0');

    // Border untuk header tabel
    $sheet->getStyle('A11:L11')->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
    ...
    foreach ($data_ptk as $ptk) {
        ...
        $sheet->setCellValue('B' . $row, $ptk['npsn']);
        $sheet->setCellValue('C' . $row, $ptk['nama_sekolah']);
        $sheet->setCellValue('D' . $row, $ptk['jenjang_pendidikan']);
        $sheet->setCellValue('E' . $row, $ptk['status_sekolah']);
        $sheet->setCellValue('F' . $row, $ptk['nama_kecamatan']);
        $sheet->setCellValue('G' . $row, $ptk['nama_kabupaten']);
        $sheet->setCellValue('H' . $row, $ptk['total_guru']);
        $sheet->setCellValue('I' . $row, $ptk['total_tendik']);
        $sheet->setCellValue('J' . $row, $ptk['total_ptk_guru_tendik']);
        $sheet->setCellValue('K' . $row, $ptk['ptk_laki']);
        $sheet->setCellValue('L' . $row, $ptk['ptk_perempuan']);
...
header('Content-Disposition: attachment;filename="laporan_ptk.xlsx"');
